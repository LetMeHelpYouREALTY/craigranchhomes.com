...
describe("Cloudflare Images transforms", () => {
  ...
  it("falls back to Next.js file-convention icons when Images is disabled", () => {
  ...
  it("uses the baked account hash by default for face-crop favicons", () => {
    delete process.env.NEXT_PUBLIC_CLOUDFLARE_IMAGES_ENABLED;
    delete process.env.NEXT_PUBLIC_CLOUDFLARE_ACCOUNT_HASH;
    delete process.env.NEXT_PUBLIC_MEDIA_CDN;

    expect(cloudflareImagesConfig().enabled).toBe(true);
    expect(agentIconUrl(180)).toContain(`imagedelivery.net/${CLOUDFLARE_IMAGES_ACCOUNT_HASH}`);
    expect(agentIconUrl(180)).toContain("gravity=face");
    expect(agentIconMetadata().apple[0]?.url).toContain("width=180,height=180");
  });
});
