 ...
 *
 * Named variant `public` is used for full stills. Favicons/avatars use
 * flexible-variant options (fit=cover, gravity=face) so we do not store
 * extra cropped copies. Do not orange-cloud the Vercel hostname.
 *
...
export function cloudflareImagesConfig(): CloudflareImagesConfig {
  ...
  const enabled =
    ...
    hash.length > 0;
  return { enabled, hash, mediaCdn };
}

export function isCloudflareImagesEnabled(): boolean {
  return cloudflareImagesConfig().enabled;
}
